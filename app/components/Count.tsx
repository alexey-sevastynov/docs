...
export function Count() {
    ...
    const [tableRows, setTableRows] = useState<TableRow[]>(() => {
        const initialRows: TableRow[] = [
            { id: 1, input: "м.Дніпро - по місту", clock: "", price: "", unit: "" },
        ];

        for (let i = 2; i <= 15; i++) {
            initialRows.push({ id: i, input: "", clock: "", price: "", unit: "" });
        }
        return initialRows;
    });
                ...
                <tbody>
                    ...
                    {calculatedRows.map((item) => (
                        <tr key={item.id}>
                            ...
                            <th>
                                <p>{item.clock === "" || item.price === "" ? "" : item.amount.toFixed(2)}</p>
                            </th>
                        </tr>
                    ))}
                </tbody>
            ...
            <table>
                <tfoot>
                    <tr>
                        ...
                        <th className="tfoot__col-3">{sumFixed.replace(".", ",")}</th>
                    </tr>
                    <tr>
                        {activeEdit ? (
