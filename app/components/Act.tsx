...
export function Act() {
    ...
    return (
        <>
            ...
            <div className="text">
                <p>
                    Ми, що нижче підписались представники Замовника {name} , з однієї сторони, і представник
                    Виконавця. Приватний підприємець Севастьянов О.В, з другої сторони, склали справжній акт
                    про те, що Виконавцем були проведені наступні роботи (надання таких послуг) за рахунком №
                    {count} від &quot;{day}&quot; {month} {year}р.:
                    <br />
                    <input
                ...
                {activeEdit ? (
                    ...
                    <>
                        <p className="edit-text">
                            Загальна вартість робіт (послуги) склала без ПДВ {price1}грн. {price2}коп.({text})
                            <img
                                onClick={() => setActivEdit(!activeEdit)}
                                className="edit"
                                src="./icon/close.png"
                        ...
                        <input
                            onClick={() => setText(numberInString(price1).replace(/\s+/g, " ").trim())}
                            className="input100"
